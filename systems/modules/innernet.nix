...
let cfg = config.services.innernet;
in {
  options.services.innernet = with types; {
    ...
    configFile = mkOption {
      ...
      type = nullOr path;
      description =
        "Path to the config file for the innernet server interface.";
    };
  ...
  config = let
    interfaceName = if cfg.configFile != null then
      builtins.head (builtins.match "/nix/store/[a-zA-Z0-9]+-([a-zA-Z_-]+).conf"
        "${cfg.configFile}")
    else
      cfg.interfaceName;
  ...
  in mkIf cfg.enable {
    assertions = [{
      ...
      message =
        "Either but not both `configFile` and `config` should be specified for innernet.";
    }];
