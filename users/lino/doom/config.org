...
Here, I set the location of my projects, i.e. parent directories of projects
that are version-controlled. The direcory structure is derived as follows:

#+begin_src bash :tangle no :results silent
~/Source/"$upstream_domain"/"$org_name_if_sensible"-"$repo_name"
#+end_src

This allows for easy per-remote configuration of certain git variables via
...
The below is needed as on NixOS, the cursor in the non-daemonized Emacs often
defaults to black.
...
Now, apply to the rest of the desktop using =pywal=
...
If you use =msmtp=, you'll likely want to add the following:
