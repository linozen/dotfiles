...
(setq bookmark-default-file "~/.dotfiles/users/lino/doom/bookmarks")
...
(setq evil-normal-state-cursor '(box "#88c0d0")
      evil-insert-state-cursor '(bar "#a3be8c")
      evil-visual-state-cursor '(hollow "#b48ead"))
...
(after! mu4e
  ...
  (set-email-account! "personal"
                      '((mu4e-sent-folder               . "/mailbox/Sent")
                        ...
                        (mu4e-refile-folder             . "/mailbox/Archive/2022")
                        ;; used when sending only plaintext (when prefixing with SPC u)
                        (mu4e-compose-signature         . "Linus Sehn\nhttps://sehn.tech"))
                      t)

  (set-email-account! "fsfe"
                      '((mu4e-sent-folder               . "/mailbox/FSFE/Sent")
                        (mu4e-drafts-folder             . "/mailbox/Drafts")
                        (mu4e-trash-folder              . "/mailbox/Trash")
                        (mu4e-refile-folder             . "/mailbox/FSFE/Archive/2022")
                        ;; used when sending only plaintext (when prefixing with SPC u)
                        (mu4e-compose-signature         . "Linus Sehn\nhttps://fsfe.org"))
                      t))
...
(after! mu4e
         (setq mu4e-maildir-shortcuts '((:maildir "/mailbox/Inbox"      :key  ?i)
                                        (:maildir "/mailbox/FSFE"       :key  ?f)
                                        (:maildir "/mailbox/FSFE/Lists" :key  ?l))))

(add-hook 'mu4e-compose-mode-hook
          (defun encrypt-per-default ()
            "Encypt message by default but allow user to toggle it off, e.g. if recipient keys are unavailable"
            (mml-secure-message-sign-encrypt)
            (org-msg-mode)))

;; (add-hook 'message-send-hook 'mml-secure-message-sign-encrypt)

(setq +org-msg-accent-color "#4c566a")
